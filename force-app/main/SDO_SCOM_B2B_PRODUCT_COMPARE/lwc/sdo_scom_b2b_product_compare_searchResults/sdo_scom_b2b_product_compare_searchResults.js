...
import productSearch from '@salesforce/apex/SDO_SCOM_B2B_Product_Compare_Search_Cont.productSearch';
import getCartSummary from '@salesforce/apex/SDO_SCOM_B2B_Product_Compare_GetInfo.getCartSummary';
import addToCart from '@salesforce/apex/SDO_SCOM_B2B_Product_Compare_GetInfo.addToCart';
import { transformData } from './sdo_scom_b2b_product_compare_dataNormalizer';
 ...
 */
export default class Sdo_scom_b2b_product_compare_searchResults extends LightningElement {
    @api _displayData;
    _isLoading = false;
    _pageNumber = 1;
    _refinements = [];
	...
	subscribeToMessageChannel() {
		this.subscription = subscribe(
			this.messageContext,
			PRODUCT_UPDATE_CHANNEL,
			(message) => this.handleProductCompareUpdate(message)
		);
	}

    // Disable/enable the [Compare] button, based on the response from the b2bProductCompareTray component
	handleProductCompareUpdate(message) {
        for(let i = 0; i < this._displayData.layoutData.length; i++) {
            let product = this._displayData.layoutData[i];

            if(message.productId === product.id) {            
                if (message.isIncluded === true) {
                    product.addToCompareDisabled = true;
                }
                else {
                    product.addToCompareDisabled = false;
                }
                break;
            }
        }
