 ...
 */
export default class Sdo_scom_b2b_product_compare_searchCard extends LightningElement {
    /**
	...
	get fields() {
		return (this.displayData.fields || []).map(({ name, value }, id) => ({
			...
			value:
				name === "Name" || name === "Description" || name === 'StockKeepingUnit'
					? value
					: `${name}: ${value}`
		}));
	}
	...
	handleAddToCompare() {
		...
		console.log('searchCard - handleAddToCompare:', this.displayData.id);
		
		const payload = {
			productId: this.displayData.id,
			isSelected: true,
			productDetails: JSON.stringify(this.displayData)
		};

		console.log("payload:", payload);

		publish(this.messageContext, PRODUCT_COMPARE_CHANNEL, payload);
	}
	...
	handleProductCompareUpdate(message) {
		...
		console.log('searchCard - handleProductCompareUpdate:', message);

		if(message.productId === this.displayData.id) {

			console.log('matching product found!');
		
			if (message.isIncluded === true) {
				this.addToCompareDisabled = true;
				this.selectedForCompare = true;
			}
			else {
